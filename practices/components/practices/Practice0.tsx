...
export default function Practice0() {
  ...
  const handleFile = async (file: File, action: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        console.error("Error capturing file");
        return;
      }
    ...
    <div className="w-full h-full grid grid-cols-2">
      <div className="w-full h-full flex flex-col pt-10 space-y-10 items-center">
        ...
        <section className="w-full text-center">
          <textarea
            className="bg-gray-100 dark:bg-gray-900 dark:text-gray-300 p-3 w-[75%] h-[25vh] rounded-md overflow-auto focus:border-blue-500"
            id="pdfText"
            value={encrypedText}
            readOnly
          />
      ...
      <div className="w-full h-full flex flex-col pt-10 space-y-10 items-center">
        ...
        <section className="w-full text-center">
          <textarea
            className="bg-gray-100 dark:bg-gray-900 dark:text-gray-300 p-3 w-[75%] h-[25vh] rounded-md overflow-auto"
            id="pdfText"
            value={decrypedText}
            readOnly
          />
